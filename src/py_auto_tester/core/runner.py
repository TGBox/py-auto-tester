...
from py_auto_tester.core.checks import CheckRecorder, build_check_api, evaluate_checks
from py_auto_tester.core.models import RunConfig, RunResult, StepResult
from py_auto_tester.core.run_artifacts import RunArtifacts, prune_runs
...
class TestRunner:
    ...
    def _write_run_index(self):
        """Laufübersicht neu erzeugen — nach dem Aufräumen, damit sie stimmt."""
        try:
            run_index.generate(self.pm.runs_dir)
        except Exception as e:
            self._log(f"[WARN] Laufübersicht konnte nicht erstellt werden: {e}")
