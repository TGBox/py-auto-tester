...
Aufgabenplaner. Braucht kein Qt und kein Display.
    ...
    py-auto-tester run demo_login --mode routine --dataset demo_users
...
from py_auto_tester.core.models import RunConfig, RunResult, SPEED_MODES, BROWSER_ENGINES, DEVICE_PROFILES
from py_auto_tester.core.runner import TestRunner, RunnerListener
from py_auto_tester.core import junit_report, run_index
...
def print_summary(result: RunResult, quiet: bool = False):
    ...
    if result.artifacts_dir:
        index_path = os.path.join(os.path.dirname(result.artifacts_dir), "index.html")
        if os.path.exists(index_path):
            print(f"  Übersicht: {index_path}")
...
def _report_load_warnings(pm: ProjectManager):
    """
    Beschädigte Projektdateien dürfen nicht still bleiben. Nach dem Lesen
    aufrufen — beim Anlegen des Managers wurde noch nichts gelesen.
    """
    for warning in pm.drain_load_warnings():
...
def cmd_list(args) -> int:
    ...
    _report_load_warnings(pm)
    return EXIT_OK


def cmd_index(args) -> int:
    """Laufübersicht neu erzeugen (passiert nach jedem Lauf automatisch)."""
    pm = ProjectManager(args.project)
    path = run_index.generate(pm.runs_dir)
    runs = run_index.collect_runs(pm.runs_dir)
    print(f"Laufübersicht mit {len(runs)} Lauf/Läufen: {path}")
    _report_load_warnings(pm)
    return EXIT_OK
...
def cmd_gui(args) -> int:
    ...
    try:
        from py_auto_tester.app import run_gui
    except ImportError as e:
        print(
            "Fehler: Die grafische Oberfläche braucht PySide6, das hier nicht "
            f"geladen werden kann ({e}).\n"
            "Installieren mit:  uv sync   bzw.  pip install PySide6",
            file=sys.stderr,
        )
        return EXIT_USAGE
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  py-auto-tester run nightly --headless --console-policy fail\n"
            "\n"
            "Ohne installiertes Paket: uv run python main.py (GUI) bzw.\n"
            "                         uv run python -m py_auto_tester run ...\n"
        ),
    )
    parser.add_argument("--project", default="project_data",
                        help="Projektordner (Standard: project_data)")

    # --project soll vor UND nach dem Unterbefehl funktionieren. SUPPRESS ist
    # hier wichtig: ohne das würde der Default des Unterbefehls einen vorher
    # angegebenen Wert wieder überschreiben.
    ...
    p_gui = sub.add_parser("gui", parents=[common], help="Grafische Oberfläche starten")
    p_gui.set_defaults(func=cmd_gui)

    p_list = sub.add_parser("list", parents=[common], help="Tests, Gruppen, Routinen und Datensätze anzeigen")
    p_list.set_defaults(func=cmd_list)

    p_index = sub.add_parser("index", parents=[common],
                             help="Laufübersicht runs/index.html neu erzeugen")
    p_index.set_defaults(func=cmd_index)

    p_run = sub.add_parser("run", parents=[common], help="Test, Gruppe oder Routine ausführen")
